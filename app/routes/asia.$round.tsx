...
export default function AsiaRound() {
  ...
  // ======= NOUVEAU : ancres de bord (au lieu du centre) =======
  const ANCHOR_OUTSET = 6; // px : petit décalage pour éviter de toucher le sprite
  ...
  // Bidules (numéros) à gauche => sortie à DROITE
  const getFromAnchor = (fromId: string) => getAnchor(fromId, 'right');

  // Symboles à droite => entrée à GAUCHE
  const getToAnchor = (toId: string) => getAnchor(toId, 'left');

  // Pendant le drag, si on survole un symbole, snap sur son ancre gauche ; sinon, suivre le curseur
      ...
      <main className="asia-screen" onMouseMove={handleMove} onMouseUp={handleCancel}>
        ...
        <div className="outlined-container" ref={containerRef}>
          ...
          <svg className="cables">
            {connections.map((c, i) => {
              const a = getFromAnchor(c.from); // bord droit du bidule
              const b = getToAnchor(c.to);     // bord gauche du symbole
              if (!a || !b) return null;
              return (
                <path
                  key={i}
            ...
            {activeFrom && (mousePos || hoverSymbol) && (() => {
              const a = getFromAnchor(activeFrom);   // sortie à droite
              const b = getHoverTargetPoint();       // curseur ou ancre gauche du symbole survolé
              if (!a || !b) return null;
              return (
                <path
                  d={makePath(a, b)}
